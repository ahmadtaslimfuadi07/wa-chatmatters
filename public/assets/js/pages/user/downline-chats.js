...
function getContactsWithLastMessage() {
  ...
  $.ajax({
    type: 'POST',
    url: base_url_ + `/user/downline/device/contacts/${uuid}`,
    dataType: 'json',
    success: function (response) {
      ...
      if (response.status) {
        ...
        $('.server-disconnect').addClass('none');
        ...
        $('.server-disconnect').removeClass('none');
      ...
      $.each(response.chats, function (_key, item) {
        if (item.number === response.phone || item.number === '0') return;
...
function getChatByPhoneNumber(phone, limit = 10, page = 0, lastId = '') {
  const base_url_ = $('#base_url').val();
  $.ajaxSetup({
    headers: {
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
  ...
  $.ajax({
    type: 'POST',
    url: base_url_ + `/user/downline/device/chats/${uuid}`,
    data: { phone, limit, page },
    dataType: 'json',
    success: function (response) {
      $('.chat-room').removeClass('justify-content-center');
      $('.chat-room').addClass('justify-content-between');
      $('.loading-messages').addClass('none');
      if (page) {
        $('.load-more-spinner').addClass('none');
      }
      $('#selected-number').removeClass('none');
      $('#selected-number').html(`+${phone}`);
      $.each(response.chats, function (_key, item) {
...
$(document).on('click', '.load-more', function () {
  const phone = $('#selected-number').html().replace('+', '');
